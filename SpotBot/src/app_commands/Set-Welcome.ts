...
export const getData = () => {
    ...
    builder
        .setName('set_welcome')
        .setDescription(
            'Allows an admin to set server-specific welcome messages.'
        )
...
export default class SetWelcomeCommand {
    ...
    public execute = async (): Promise<void> => {
        ...
        if (this.interaction.options.get('message') === null) {
            response =
                `Please supply the message you would like to use when welcoming new members.\r\n` +
                `The next message entered by the command issuer will be saved as the welcome message.`;
        }

        await this.interaction.reply(response);

        let setResponse = await this.services.welcomeService.setWelcomeMessage(
